...
function () {
  function Customizator() {
    ...
    this.colorPicker = document.createElement('input'); // кнопка для сброса значений

    this.clear = document.createElement('div'); // Получаем данные из Local Storage. Если там еще нечего нет,то получим значение по-умолчанию
  ...
  _createClass(Customizator, [{
    ...
    value: function onScaleChange(e) {
      ...
      var body = document.querySelector('body'); // проверяем, что кнопка, в которую ткнули, имеет параметр value
        ...
        element.childNodes.forEach(function (node) {
          ...
          if (node.nodeName === '#text' && node.nodeValue.replace(/\s+/g, "").length > 0) {
            ...
            if (!node.parentNode.getAttribute('data-fs')) {
              ...
              var value = window.getComputedStyle(node.parentNode, null).fontSize; // создаем новый дата-атрибут и записываем в него исходный размер шрифта

              node.parentNode.setAttribute('data-fs', +value.replace(/px/g, "")); // умножаем размер шрифта на полученный из кнопки масштаб

              node.parentNode.style.fontSize = node.parentNode.getAttribute('data-fs') * _this2.scale + "px"; // если исходный размер шрифта уже записывали в дата-атрибут
            } else {
              node.parentNode.style.fontSize = node.parentNode.getAttribute('data-fs') * _this2.scale + "px";
    ...
    value: function render() {
      ...
      this.injectStyle(); // вызываем метод, отображающий цвет фона из LocalStorage. Либо дефолтное значение, либо то, что было ранее туда записано

      this.setBgColor();
      this.onScaleChange(); // создаем кнопки (в виде инпутов) и главную панель

      var scaleInputS = document.createElement('input'),
          scaleInputM = document.createElement('input'),
          panel = document.createElement('div'); // добавляем кнопки на главную панель

      panel.append(this.btnBlock, this.colorPicker, this.clear); // вставляем "крестик"

      this.clear.innerHTML = "&times";
      this.clear.classList.add('clear'); // добавляем стили, заданные в css

      scaleInputS.classList.add('scale_btn');
      scaleInputM.classList.add('scale_btn');
      this.btnBlock.classList.add('scale');
      this.colorPicker.classList.add('color'); // меняем типы инпутов и задаем им дефолтные значения
